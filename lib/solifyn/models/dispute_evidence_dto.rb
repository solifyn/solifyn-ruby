  ...
  class DisputeEvidenceDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Solifyn::DisputeEvidenceDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Solifyn::DisputeEvidenceDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'customer_communication')
        self.customer_communication = attributes[:'customer_communication']
      end

      if attributes.key?(:'refund_policy')
        self.refund_policy = attributes[:'refund_policy']
      end
    ...
    def hash
      [cancellation_policy, customer_communication, refund_policy, uncategorized].hash
    end
