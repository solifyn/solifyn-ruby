  ...
  class CreateCheckoutDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Solifyn::CreateCheckoutDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Solifyn::CreateCheckoutDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'checkout_data')
        self.checkout_data = attributes[:'checkout_data']
      end
      ...
      if attributes.key?(:'aff')
        self.aff = attributes[:'aff']
      end
    ...
    def hash
      [product_id, currency, quantity, discount_code, custom_price, customer_email, checkout_data, custom_fields, aff, checkout_id].hash
    end
