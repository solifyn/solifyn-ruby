...
module Solifyn
  class WebhookSubscriptionPayload
    # The subscription ID.
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Solifyn::WebhookSubscriptionPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Solifyn::WebhookSubscriptionPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, status, cancel_at_period_end, renewal_period_start, renewal_period_end, currency, amount, customer_id, customer_email, customer_name, product_id, product_title, created_at, updated_at].hash
    end
